...
# Discriminator
class D(nn.Module):
    def __init__(self, in_nc, out_nc, ndf=64):
        ...
        self.non_local_block = NONLocalBlock2D(in_channels=ndf * 8)
...
class Discriminator(nn.Module):
    def __init__(self, input_nc, image_size=256, conv_dim=64, repeat_num=6):
        ...
        for i in range(1, repeat_num):
            ...
            layers.append(nn.LeakyReLU(0.01, inplace=True))
            curr_dim = curr_dim * 2
        layers.append(NONLocalBlock2D(curr_dim))
...
class sketchGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, conv_dim=64, repeat_group_num=3):
        ...
        layers.append(NONLocalBlock2D(in_channels=curr_dim))
        self.down = nn.Sequential(*layers)
        # non local block

        layers = []
        # Bottleneck
        for i in range(repeat_group_num):
            layers.append(ResidualGroup(curr_dim))
        ...
        self.face_reg = nn.Sequential(*layers)'''
...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        # chanel-wise attention
        self.attention = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(dim_in, int(dim_in / 16), kernel_size=1, stride=1, padding=0, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(int(dim_in / 16), dim_out, kernel_size=1, stride=1, padding=0, bias=False),
            nn.Sigmoid()
        )

    def forward(self, x):
        main = self.main(x)
        attention = self.attention(main)
        main = attention * main
        return x + main
